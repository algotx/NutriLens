...
// ── Saved food card ───────────────────────────────────────────────────────────
function SavedFoodCard({ food, mealType, onLog, onDelete }) {
  return (
    <View style={styles.savedCard}>
      <View style={{ flex: 1 }}>
        <Text style={styles.savedName}>{food.food_name}</Text>
        <Text style={styles.savedMeta}>
          {food.serving_size ? `${food.serving_size} · ` : ''}{Math.round(food.calories)} kcal
        </Text>
...
export default function LogMeal() {
  ...
  const [tab, setTab] = useState('manual'); // 'manual' | 'saved'
  ...
  // ── save current form as a saved food ──────────────────────────────────────
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.tabRow}>
          <TouchableOpacity style={[styles.tabBtn, tab === 'manual' && styles.tabBtnActive]} onPress={() => setTab('manual')} activeOpacity={0.8}>
            <Ionicons name="create-outline" size={15} color={tab === 'manual' ? colors.primaryLight : colors.textMuted} />
          ...
          <TouchableOpacity style={[styles.tabBtn, tab === 'saved' && styles.tabBtnActive]} onPress={() => setTab('saved')} activeOpacity={0.8}>
            <Ionicons name="bookmark-outline" size={15} color={tab === 'saved' ? colors.primaryLight : colors.textMuted} />
          ...
          <>
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Ionicons name="fast-food-outline" size={16} color={colors.primaryLight} />
            ...
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Ionicons name="nutrition-outline" size={16} color={colors.primaryLight} />
            ...
            {filtered.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySub}>
                  {search ? 'Try a different search' : 'Fill in the Manual Entry form and tap "Save food for later"'}
                </Text>
              </View>
            ) : (
              filtered.map((food) => (
...
const styles = StyleSheet.create({
  ...
  orb: { position: 'absolute', top: -40, right: -60, width: 200, height: 200, borderRadius: 100, backgroundColor: colors.primaryGlow },
  ...
  typeCard: { flex: 1, backgroundColor: colors.card, borderRadius: radius.md, borderWidth: 1.5, borderColor: colors.border, padding: spacing.sm, alignItems: 'center', gap: 6 },
  ...
  tabRow: { flexDirection: 'row', backgroundColor: colors.card, borderRadius: radius.md, padding: 4, marginBottom: spacing.lg, borderWidth: 1, borderColor: colors.border },
  ...
  tabBadge: { backgroundColor: colors.primary, borderRadius: 10, paddingHorizontal: 6, paddingVertical: 1 },
  ...
  errorBox: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(239,68,68,0.1)', borderRadius: radius.sm, padding: spacing.sm, marginBottom: spacing.md },
  errorText: { color: colors.danger, fontSize: 13, flex: 1 },

  saveForLaterBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: `${colors.accent}15`, borderRadius: radius.md, paddingVertical: 12, marginBottom: spacing.md, borderWidth: 1, borderColor: `${colors.accent}30` },
  saveForLaterText: { color: colors.accent, fontWeight: '600', fontSize: 14 },

  searchWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.card, borderRadius: radius.md, paddingHorizontal: spacing.md, paddingVertical: 10, marginBottom: spacing.md, borderWidth: 1, borderColor: colors.border },
  searchInput: { flex: 1, color: colors.text, fontSize: 14 },

  savedCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.card, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm, borderWidth: 1, borderColor: colors.border, ...shadow.sm },
  savedName: { color: colors.text, fontWeight: '600', fontSize: 14 },
  savedMeta: { color: colors.textMuted, fontSize: 12, marginTop: 2 },
  savedMacroRow: { flexDirection: 'row', gap: spacing.sm, marginTop: 4 },
  savedMacro: { color: colors.textMuted, fontSize: 11 },
  savedActions: { flexDirection: 'row', alignItems: 'center', gap: spacing.md },
  savedLogBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.primaryGlow, borderRadius: radius.full, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: `${colors.primary}40` },
  savedLogText: { color: colors.primaryLight, fontWeight: '700', fontSize: 12 },
  ...
  emptySub: { color: colors.textMuted, fontSize: 13, textAlign: 'center', paddingHorizontal: spacing.lg },
});
